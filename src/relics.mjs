...
const regions=['Verdantglade','Cinder Ridge','Aqualis','Loong Haven'];
const rarityOrder=['Mythic','Legendary','Epic','Rare','Uncommon','Common'];
const regionOf=relic=>regions.find(region=>relic.region===region||relic.region?.startsWith(region+' '))||'other';
...
export function initialize(){
 ...
 <nav class="relicRegionTabs" id="relicRegionTabs" aria-label="Relic regions">${[...regions,'other','all'].map(region=>`<button type="button" data-relic-region="${region}" aria-pressed="${region===filters.region}">${region==='other'?'Other':region==='all'?'All regions':region}</button>`).join('')}</nav><nav class="relicZoneTabs" id="relicZoneTabs" aria-label="Exact relic zone"></nav>
 ...
 <p id="relicTargetNote" hidden>Missing relics with verified Destiny Fruit availability. Zone counts follow your current filters.</p><p class="relicStorageStatus" id="relicStorageStatus" role="status"></p><p id="relicResultsStatus" role="status" aria-live="polite"></p><div id="relicGroups"></div><p class="relicCoverage">Through Loong Haven Relic II: ${relics.length} relics · ${relics.filter(r=>r.zone).length} relics with verified exact zones · ${relics.filter(r=>r.destinyFruit==null).length} Destiny Fruit routes still unverified. Later releases are hidden.</p><p class="relicFootnote">Select a relic for all acquisition methods and source links. Unverified does not mean unavailable.</p>
 ...
 function card(r){
  ...
  return `<article class="relicCard${owned.has(r.id)?' isOwned':''}" data-relic-id="${esc(r.id)}" data-rarity="${rarity}"><button class="relicCardOpen" type="button" data-relic-open="${esc(r.id)}" aria-label="View ${esc(r.name)} acquisition details"><span class="relicImage">${r.image?`<img src="${esc(r.image)}" alt="${esc(r.name)}" width="120" height="120" loading="lazy">`:'<span aria-label="Icon unavailable">◇</span>'}</span><strong class="relicName">${esc(r.name)}</strong><span class="relicTileMeta">${esc(r.element||'Element unverified')}</span><span class="relicTileZone">${esc(r.zone||(filters.region==='all'?r.region:'Zone unverified'))}</span></button><label class="relicOwned"><input type="checkbox" data-owned="${esc(r.id)}" ${owned.has(r.id)?'checked':''} aria-label="Own ${esc(r.name)}"><span>${owned.has(r.id)?'Owned':'Missing'}</span></label></article>`;
 }
 ...
 function renderZones(){
  ...
  $('relicZoneTabs').innerHTML=values.map(zone=>{const label=zone==='all'?'All zones':zone==='unknown'?'Unverified':filters.region==='all'?zone:zone.replace(filters.region+' ','');return `<button type="button" data-relic-zone="${esc(zone)}" aria-label="${esc(zone==='unknown'?'Unverified exact zone':zone)}" aria-pressed="${zone===filters.zone}">${esc(label)}</button>`;}).join('');
 }
 function render(){
 ...
 host.addEventListener('click',e=>{
  ...
  const zone=e.target.closest('[data-relic-zone]');if(zone){filters.zone=zone.dataset.relicZone;render();}
