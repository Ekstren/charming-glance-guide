...
export function initialize(){
 ...
 function renderZones(){
  ...
  $('relicZoneTabs').innerHTML=values.map(zone=>{const label=zone==='all'?'All zones':zone==='unknown'?'No fruit zone listed':filters.region==='all'?zone:zone.replace(filters.region+' ','');return `<button type="button" data-relic-zone="${esc(zone)}" aria-label="${esc(zone==='unknown'?'No Destiny Fruit zone listed':zone)}" aria-pressed="${zone===filters.zone}">${esc(label)}</button>`;}).join('');
 }
 function render(){
