...
export function initialize(){
 ...
 host.innerHTML=`<nav class="relicRegionTabs" id="relicRegionTabs" aria-label="Relic regions">${[...regions,'other','all'].map(region=>`<button type="button" data-relic-region="${region}" aria-pressed="${region===filters.region}">${region==='other'?'Other':region==='all'?'All regions':region==='Loong Haven II'?'Loong Haven II · Upcoming':region}</button>`).join('')}</nav>
 <header class="relicHeader"><div><h1>Relic Gallery</h1><p>Your collection, one region at a time.</p><div class="relicModes" role="group" aria-label="Relic view"><button type="button" data-relic-mode="collection" aria-pressed="true">Collection</button><button type="button" data-relic-mode="targets" aria-pressed="false">Destiny Fruit Targets</button></div></div><div class="relicProgress"><strong id="relicProgressText"></strong><progress id="relicProgressBar" aria-label="Overall relic collection"></progress><small>Saved in this browser on this device</small></div></header>
