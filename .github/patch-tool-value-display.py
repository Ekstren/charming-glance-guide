...
# Show the projected inventory left after the recommended S1 upgrade spend.
...
'''
    new_balance = r'''  /* RAW_REMAINING_DISPLAY_V1: visible balance is only material-equivalent left
  ...
  function setRawRemaining(id,cost,available,unitLabel=''){
    ...
    el.hidden=false;
    el.textContent=`${fmt(left)}${unitLabel?` ${unitLabel}`:''} left`;
  }
...
'''
    ...
    new_ore = r'''      setRawRemaining('oreBalance',plan.oreCost,resources.ore);'''
    if old_ore not in text:
        raise SystemExit('feasible Ore balance render block not found')
    text = text.replace(old_ore, new_ore, 1)
    changed = True

# Existing V1 installs: remove the redundant word "raw" from Ore / Essence / Sand rows.
if 'RAW_REMAINING_DISPLAY_V1' in text and 'RAW_REMAINING_NO_RAW_LABEL_V2' not in text:
    old_v1 = r'''  /* RAW_REMAINING_DISPLAY_V1: visible balance is only raw/material-equivalent left
     after the S1 upgrade spend. Reserve bookkeeping stays internal. */
  ...
  function setTreatBalance(id,cost,resources){ setRawRemaining(id,cost,resources?.treat,'basic-eq.'); }'''
    new_v2 = r'''  /* RAW_REMAINING_DISPLAY_V1: visible balance is only material-equivalent left
     after the S1 upgrade spend. Reserve bookkeeping stays internal.
  ...
  function setTreatBalance(id,cost,resources){ setRawRemaining(id,cost,resources?.treat,'basic-eq.'); }'''
    if old_v1 not in text:
        raise SystemExit('existing raw remaining V1 block not found')
    text = text.replace(old_v1, new_v2, 1)
    old_ore_v1 = "setRawRemaining('oreBalance',plan.oreCost,resources.ore,'raw');"
    if old_ore_v1 not in text:
        raise SystemExit('existing Ore raw label call not found')
    text = text.replace(old_ore_v1, "setRawRemaining('oreBalance',plan.oreCost,resources.ore);", 1)
    changed = True

if changed:
    path.write_text(text, encoding='utf-8')
    print('Applied tool display, compact Fantomon card, and simplified remaining balances.')
else:
    print('Tool display, Fantomon card, and remaining balances already current.')
