...
# Old reserve-state migration is no longer meaningful.
...
# Remove S1->S2 reserve construction helpers in one block.
...
# Full projected materials are directly available to the active-season optimizer.
...
# Auto Stamina / Realm topups no longer have a reserve-aware branch.
...
# Delete the reserve-target / reserve-aware Realm helper family.
...
# Full raw/material-equivalent supply participates in S1 optimization.
...
# Remove all reserve balance rendering. Remaining means simply post-plan remaining.
...
# Material projections display the actual full projected pools.
...
# Remove season-chrome handling for controls that no longer exist.
s=re.sub(r"\n    \['reserveS2Essence','reserveS2Sand','reserveS2Treats'\]\.forEach\(id=>\{.*?\n    \}\);",'',s,flags=re.S)

# Explain that the optimizer method is shared while scoring stays season-specific.
...
# Guard S1 scoring constants explicitly: do NOT import S2 values into the S1 calculator.
...
banned=['reserveS2Essence','reserveS2Sand','reserveS2Treats','season2SkillEssenceReserve','season2RelicSandReserve','season2FantomonTreatReserve','applySeasonTransitionReserves','reserveTargetFor','reserveAwareRealmTopupFor','rawFirstReserveSplit','S2 Skill reserve:','S2 Relic Sand reserve:','S2 Fantomon Treat reserve:']
for x in banned:
    if x in s:
