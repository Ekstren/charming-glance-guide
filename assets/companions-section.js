...
var SxsCompanions = (() => {
  ...
  // assets/companions.js
  var require_companions = __commonJS({
    "assets/companions.js"() {
      ...
      (() => {
        ...
        function loadPrefs() {
          ...
          try {
            const r = localStorage.getItem(roleStorage);
            if (r === "dps" || r === "heals") sageRole = r;
          } catch (_) {
          }
          const list = currentSeason === "s2" ? S2_CLASSES : S1_CLASSES;
          let saved = "";
          try {
            saved = localStorage.getItem(classStorage[currentSeason]) || "";
          } catch (_) {
          }
        ...
        function savePrefs() {
          try {
            localStorage.setItem(classStorage[currentSeason], currentClass);
            localStorage.setItem(roleStorage, sageRole);
          } catch (_) {
          }
        }
      ...
      <div class="companionSources">Research basis · <a href="https://www.prydwen.gg/sword-x-staff/guides/beginner-guide" target="_blank" rel="noreferrer">Prydwen Companion basics ↗</a> · <a href="https://www.lootbar.com/blog/en/companion-upgrade-guide-sword-x-staff.html" target="_blank" rel="noreferrer">LootBar breakpoint guide ↗</a> · <a href="https://lootandwaifus.com/sword-x-staff-companion-database/" target="_blank" rel="noreferrer">Loot & Waifus Companion DB ↗</a> · <a href="https://mobi.gg/en/tips/sword-x-staff-companions/" target="_blank" rel="noreferrer">Mobi class companion guide ↗</a></div>`;
        ...
        function setup() {
          ...
          loadPrefs();
          $c("companionClassTabs")?.addEventListener("click", (e) => {
            const b = e.target.closest("[data-companion-class]");
            if (!b) return;
            if (b.dataset.companionClass === currentClass) return;
            currentClass = b.dataset.companionClass;
            render();
          });
          $c("companionContent")?.addEventListener("click", (e) => {
            const b = e.target.closest("[data-companion-role]");
            if (!b) return;
            sageRole = b.dataset.companionRole;
            render();
          });
          render();
        }
  ...
  // src/companions.mjs
  var companions_exports = {};
  __export(companions_exports, {
    initialize: () => initialize
  });
  init_define_GUIDE_BUILD_IDS();
  var import_companions = __toESM(require_companions(), 1);
  function initialize() {
  }
  return __toCommonJS(companions_exports);
})();
