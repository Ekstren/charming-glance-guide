...
if old not in s:
    raise SystemExit('shop panel V2 block not found')
s = s.replace(old, new, 1)

old_js = '''    if($('shopRefreshEstimate')){
...
if old_js not in s:
    raise SystemExit('shop render V2 block not found')
s = s.replace(old_js, new_js, 1)

css = '''
/* DAILY_SHOP_ESTIMATE_V3: compact two-column shop planner */
...
print('patched Daily Shop panel to compact V3')
