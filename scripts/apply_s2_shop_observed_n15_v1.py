...
new_block = r'''  // CG_S2_SHOP_OBSERVED_N15_V1
  // Charming Glance S2 shop averages measured from fifteen observed shop pages.
  // Sep. 9 screenshots added refreshes 7-10. Refresh 7 had sold-out wood/ingot slots only;
  // all tracked Raw Ore / Battle Essence / Chrono Sand / Treat slots remained visible.
  ...
  const S2_SHOP_OBSERVED_PAGES=Object.freeze([
    ...
    // Sep. 9 · Vendor refresh 7: blue ore 1,650; no Battle Essence; purple Sand 1,650+1,350;
    // granular Rare Sand 330+270; Basic Treats 180. Sold top row was untracked wood/ingots.
    ...
    // Sep. 9 · Vendor refresh 9. Includes two Basic Treat bundles (180 + 165).
...
pattern = r"  // CG_S2_SHOP_OBSERVED_N(?:4|7|8|9|10|11|15)_V1\n.*?(?=\n  function applyS2ScoringStartDefaults\(\))"
match = re.search(pattern, s, flags=re.S)
if not match:
    raise SystemExit('Could not find observed S2 shop block')
current = match.group(0)
if 'CG_S2_SHOP_OBSERVED_N15_V1' not in current:
    s = s[:match.start()] + new_block.rstrip() + s[match.end():]
else:
    # Normalize to the exact maintained n15 block if a prior partial application exists.
    s = s[:match.start()] + new_block.rstrip() + s[match.end():]
...
method_replacement = (
    ...
    'The current sample is <b>15 observed shop pages</b>: average <b>2,540 Raw Ore</b>, <b>2,090 Battle Essence</b>, '
    '<b>1,500 regular Chrono Sand</b>, <b>196 Rare Chrono Sand</b>, and <b>189.7 Basic Treats</b> per page. '
    'Rare Chrono Sand remains separate in the sample data and is converted at <b>5 regular Sand per Rare Sand</b> only when the optimizer needs a single Sand-equivalent total, giving <b>2,480 Sand-equivalent per page</b> at the current n=15 average. '
    ...
    'This sample will be expanded as more Charming Glance shop screenshots are collected.</p>'
)
...
for needle in [
    'CG_S2_SHOP_OBSERVED_N15_V1',
    'Object.freeze({ore:1650,essence:0,sand:3000,rareSand:600,treat:180})',
    'Object.freeze({ore:1350,essence:1650,sand:2850,rareSand:270,treat:180})',
    'Object.freeze({ore:3000,essence:1200,sand:1650,rareSand:0,treat:345})',
    'Object.freeze({ore:3000,essence:1350,sand:2850,rareSand:0,treat:110})',
    'sand:a.sand+a.rareSand*SAND_BLUE_EQ',
    '15 observed shop pages',
    '2,540 Raw Ore',
    '2,090 Battle Essence',
    '1,500 regular Chrono Sand',
    '196 Rare Chrono Sand',
    '189.7 Basic Treats',
    '2,480 Sand-equivalent per page'
]:
    if needle not in s:
        raise SystemExit(f'Missing expected n15 shop marker: {needle}')

path.write_text(s, encoding='utf-8')
print('Applied Charming Glance S2 observed shop averages (n=15).')
