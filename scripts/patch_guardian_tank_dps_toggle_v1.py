...
def patch_meta_source(text: str) -> str:
    ...
    # The META_CSS/META_JS raw strings share the same anchors as the generated runtime.
    text = patch_meta_runtime(text)
    # The event-handler injection is represented as a Python string in this source, so add Guardian handling there too.
    if "data-guardian-mode" not in text.split('def disable_dominator_card_filter', 1)[0]:
        # This should not normally happen because META_JS now contains it; leave a hard failure if source format changes.
        raise RuntimeError('Guardian meta controls did not land in meta source')
    return text
