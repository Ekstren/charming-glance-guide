...
try{
 ...
 const summarize=rows=>({averageMs:median(rows.map(r=>r.averageMs)),worstMs:median(rows.map(r=>r.worstMs)),loadMs:median(rows.map(r=>r.loadMs)),longestTaskMs:Math.max(0,...rows.flatMap(r=>r.longTasks))});
